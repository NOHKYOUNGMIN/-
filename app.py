...
def load_dd_maps(version: str = DD_VER, locale: str = "ko_KR"):
    # 아이템
    ...
    # 룬 (이건 버전 경로가 다름: /cdn/img/ 하위 perk-images)
    ...
    for tree in rune_json:
        rune_name_to_icon[tree["name"]] = tree["icon"]  # 스타일 이름(지배/결의 등)
        for slot in tree["slots"]:
            for rune in slot["runes"]:
                rune_name_to_icon[rune["name"]] = rune["icon"]  # 개별 룬(집중 공격 등)
    return item_name_to_img, spell_name_to_img, rune_name_to_icon

def dd_item_url(name: str, mp: dict) -> str | None:
    f = mp.get(name)
    return f"https://ddragon.leagueoflegends.com/cdn/{DD_VER}/img/item/{f}" if f else None

def dd_spell_url(name: str, mp: dict) -> str | None:
    f = mp.get(name)
    return f"https://ddragon.leagueoflegends.com/cdn/{DD_VER}/img/spell/{f}" if f else None

def dd_rune_url(name: str, mp: dict) -> str | None:
    f = mp.get(name)
    return f"https://ddragon.leagueoflegends.com/cdn/img/{f}" if f else None  # /cdn/img/perk-images/...
...
st.sidebar.write(":mag: 자동 검색:", auto_path if auto_path else "없음")
...
# ========== ✅ 추천 세팅 (아이콘 그리드) — HTML 없이 Streamlit 레이아웃 ==========
st.subheader("추천 세팅 (아이콘)")
item_map, spell_map, rune_map = load_dd_maps(DD_VER, "ko_KR")

def show_item_grid(stats_df: pd.DataFrame, n_cols: int = 5, title: str = "아이템"):
    st.markdown(f"**{title}**")
    box = st.container()                 # ✅ 컨테이너로 격리
    cols = box.columns(n_cols, gap="small")
    for i, row in stats_df.iterrows():
        with cols[i % n_cols]:
            url = dd_item_url(row["item"], item_map)
            if url:
            ...
            if pd.notna(wr) and pd.notna(pr):
                st.caption(f"WR {wr}% · PR {pr}%")
    st.divider()

def show_spell_pairs_grid(sp_df: pd.DataFrame, n_cols: int = 5):
    st.markdown("**스펠 조합**")
    wrap = st.container()
    ...
    for i, row in sp_df.iterrows():
        with cols[i % n_cols]:
            ...
            u1, u2 = dd_spell_url(s1, spell_map), dd_spell_url(s2, spell_map)
...
def show_rune_pairs_grid(rn_df: pd.DataFrame, n_cols: int = 5):
    st.markdown("**룬 조합(메인/보조)**")
    wrap = st.container()
    cols = wrap.columns(n_cols, gap="small")
    for i, row in rn_df.iterrows():
        with cols[i % n_cols]:
            c1, c2 = st.columns([1,1])
            r1, r2 = str(row["rune_core"]), str(row["rune_sub"])
            u1, u2 = dd_rune_url(r1, rune_map), dd_rune_url(r2, rune_map)
...
# 아이템 집계(슬롯 무시)
def _item_stats_for_icons(sub: pd.DataFrame) -> pd.DataFrame:
    item_cols_local = [c for c in sub.columns if c.startswith("item")]
    rec = []
    for c in item_cols_local:
    ...
    # 픽률 계산(해당 챔프의 매치 수 기준)
    denom = sub["matchId"].nunique() if "matchId" in sub.columns else len(sub)
    g["pick_rate"] = (g["total_picks"]/denom*100).round(2) if denom else 0
    g = g.sort_values(["win_rate","pick_rate"], ascending=[False,False])
    return g

# 상단 아이콘 보드용 데이터 생성
try:
    item_top = _item_stats_for_icons(dfc).head(15) if len(dfc) else pd.DataFrame(columns=["item"])
    if not item_top.empty:
        show_item_grid(item_top, n_cols=5, title="아이템")
except Exception:
    pass

if {"spell1_final","spell2_final"}.issubset(dfc.columns) and dfc["spell1_final"].str.strip().any():
    sp_df = (dfc.groupby(["spell1_final","spell2_final"])
             .agg(total_games=("matchId","count"), wins=("win_clean","sum"))
             .reset_index()
             .rename(columns={"spell1_final":"spell1","spell2_final":"spell2"}))
    sp_df["win_rate"] = (sp_df["wins"]/sp_df["total_games"]*100).round(2)
    denom = dfc["matchId"].nunique() if "matchId" in dfc.columns else len(dfc)
    sp_df["pick_rate"] = (sp_df["total_games"]/denom*100).round(2) if denom else 0
    sp_df = sp_df.sort_values(["win_rate","pick_rate"], ascending=[False,False]).head(10)
    if not sp_df.empty:
        show_spell_pairs_grid(sp_df, n_cols=5)
else:
    st.caption("스펠 아이콘: 스펠 이름이 부족하여 생략")

if {"rune_core","rune_sub"}.issubset(dfc.columns) and dfc["rune_core"].str.strip().any():
    rn_df = (dfc.groupby(["rune_core","rune_sub"])
             .agg(total_games=("matchId","count"), wins=("win_clean","sum"))
             .reset_index())
    rn_df["win_rate"] = (rn_df["wins"]/rn_df["total_games"]*100).round(2)
    denom = dfc["matchId"].nunique() if "matchId" in dfc.columns else len(dfc)
    rn_df["pick_rate"] = (rn_df["total_games"]/denom*100).round(2) if denom else 0
    rn_df = rn_df.sort_values(["win_rate","pick_rate"], ascending=[False,False]).head(10)
    if not rn_df.empty:
        show_rune_pairs_grid(rn_df, n_cols=5)
else:
    st.caption("룬 아이콘: 룬 정보가 부족하여 생략")
